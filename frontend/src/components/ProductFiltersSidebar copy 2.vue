...
<div class="space-y-2">
  ...
  <div class="flex flex-col gap-3 border-l-2 border-emerald-600/30 pl-3">

    <!-- 🎨 COR -->
    <div>
      <p class="text-xs font-semibold text-gray-700 dark:text-gray-300 m-0 mb-1">
        Cor
      </p>

      <div v-if="facets.colors.length === 0" class="text-xs text-gray-500">
    ...
    <div>
      ...
      <div v-if="facets.sizes.length === 0" class="text-xs text-gray-500">
      ...
      <div v-else class="max-h-40 overflow-y-auto space-y-1 pr-1">
        <label
          ...
          :key="s"
          class="inline-flex items-center gap-1 px-2 py-1 rounded border text-xs cursor-pointer"
          :class="modelValue.sizes.includes(s)
    ...
    <div>
      ...
      <div v-if="facets.brands.length === 0" class="text-xs text-gray-500">
        ...
        <div class="max-h-40 overflow-y-auto pr-1">
          <div class="flex flex-wrap gap-1">
            <label
              ...
              :key="v"
              class="inline-flex items-center gap-1 px-2 py-1 rounded border text-xs cursor-pointer"
              :class="(modelValue.attributeValues[row.facet.key] ?? []).includes(v)
                ? 'border-emerald-600 bg-emerald-50'
                : 'border-gray-300'"
            >
              <Checkbox
      ...
      <div class="max-h-40 overflow-y-auto pr-1">
        <div class="flex flex-wrap gap-1">
          <label
            ...
            :key="v"
            class="inline-flex items-center gap-1 px-2 py-1 rounded border text-xs cursor-pointer"
            :class="(modelValue.attributeValues[attr.key] ?? []).includes(v)
              ? 'border-emerald-600 bg-emerald-50'
              : 'border-gray-300'"
          >
            <Checkbox
